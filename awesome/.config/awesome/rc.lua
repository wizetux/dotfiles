...
local tasklist_buttons = gears.table.join(
  ...
  awful.screen.connect_for_each_screen(function(s)
    ...
    s.mylayoutbox:buttons(gears.table.join(
      ...
      local rightTopbarLayout = wibox.layout.fixed.horizontal()
      rightTopbarLayout:add(mykeyboardlayout)
      rightTopbarLayout:add(wallpaper_widget({
        directory = "/home/wizetux/wallpaper/"
      }))
      rightTopbarLayout:add(net_widgets.indicator({
        interfaces = {"tun0"},
        timeout = 5,
        hidedisconnected = true,
        skipvpncheck = true
      }))
      rightTopbarLayout:add(cpu_widget())
      rightTopbarLayout:add(ram_widget())
      local batteryLayout = wibox.layout.fixed.horizontal();
      rightTopbarLayout:add(batteryLayout)
      rightTopbarLayout:add(wibox.widget.systray())
      rightTopbarLayout:add(s.mylayoutbox)

      -- Determine if we have acpi installed, and if so, add the battery widget
      awful.spawn.easy_async_with_shell("command -v acpi", function(out)
        if string.len(out) > 0 then
          batteryLayout:add(batteryarc_widget({
